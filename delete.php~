...
<html>
 <head>
   <title> Admin Doctor Delete </title>
  ...
  <div id="full">
    ...
    <div id="fo">
         <center> <h1 style="color:red;opacity:1;"> Available Doctors </h1></center>
          <?php
           echo "<center><table>";
           echo "<tr><th>Name</th><th>Email</th><th>Mobile</th><th>Address</th><th>Specilist</th><th>Delete</th></tr>";
           $a=0;
           while($row1=$result->fetch_array())
            {
              $a=1;
              echo "<tr><td>$row1[2] $row1[3] $row1[4]</td><td> $row1[5]</td><td> $row1[6]</td><td>$row1[7] $row1[8] $row1[9]</td><td>$row1[10]</td><td><form action='delete_back.php' method='post' ><input type='text' value='$row1[0]' style='display:none' name='id'><input type='submit'value='Delete Doctor'></a></form></td></tr>";
            }
            echo "</table></center>";
            if($a==0)
             {
